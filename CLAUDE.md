...
* **CGo is required** (`CGO_ENABLED=1`, the default) — the `src/treesitter` package
  links the tree-sitter C runtime + bundled grammars for `idit string`/`idit comment`.
...
* tree-sitter grammar modules are pinned to a language ABI the runtime accepts
  ...
  `TestRegistryGrammarsParse` test in `src/treesitter` fails loudly if a version
