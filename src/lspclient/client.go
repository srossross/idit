...
func New(opts Options) (*Client, error) {
	//nolint:gosec // launching the language server the workspace config specifies is the point
	...
	c.cmd = cmd
	go func() { _ = c.cmd.Wait() }() // reap the child; exit is observed via stdout EOF
...
func (c *Client) Notify(method string, params any) {
	// Fire-and-forget: a send failure means the server's gone, which the caller
	// learns via Exited().
	_ = c.send(rpcMessage{JSONRPC: "2.0", Method: method, Params: mustRaw(params)})
}
	...
	case hasID && hasMethod:
		...
		if msg.Method == "workspace/applyEdit" {
			...
			if waiter != nil {
				...
				_ = json.Unmarshal(msg.Params, &p) // malformed → nil edit handed off
				waiter <- p.Edit
...
func (c *Client) replyResult(id *json.RawMessage, result any) {
	...
	_ = c.send(rpcMessage{JSONRPC: "2.0", ID: id, Result: raw})
}

func (c *Client) replyError(id *json.RawMessage, rpcErr *RpcError) {
	_ = c.send(rpcMessage{JSONRPC: "2.0", ID: id, Error: rpcErr})
}
