...
// KnownKind reports whether name is a canonical SymbolKind name.
func KnownKind(name string) bool {
	for _, n := range symbolKind {
		if n == name {
			return true
		}
	}
...
// KindNames returns the canonical SymbolKind names, sorted.
func KindNames() []string {
	names := make([]string, 0, len(symbolKind))
	for _, n := range symbolKind {
