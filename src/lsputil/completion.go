...
func CompletionItems(result json.RawMessage) []CompletionItem {
	...
	if trimmed[0] == '[' {
		...
		_ = json.Unmarshal(result, &arr) // malformed → empty list
		return arr
	}
	var list completionList
	_ = json.Unmarshal(result, &list) // malformed → empty list
	return list.Items
}
...
func IsIncomplete(result json.RawMessage) bool {
	...
	var list completionList
	_ = json.Unmarshal(result, &list) // malformed → not incomplete
